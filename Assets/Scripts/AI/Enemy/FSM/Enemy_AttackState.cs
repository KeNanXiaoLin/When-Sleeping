...
public class Enemy_AttackState : BaseState
{
    ...
    public override void Enter()
    {
        ...
        enemyObj.Animator.SetTrigger(Setting.PlayerAnimationParameter_Enemy_Attack);
    }
    ...
    public override void Update()
    {
        ...
        AnimatorStateInfo attackStateInfo = owner.Animator.GetCurrentAnimatorStateInfo(0);

        // 2. 判断：是否正在播放攻击动画 + 动画已播放完成（normalizedTime >= 1）
        if (attackStateInfo.IsName(Setting.AnimationName_Enemy_Attack) && attackStateInfo.normalizedTime >= 1.0f)
        {
